...
const NewEntry = () => {
  ...
  const entrySubmitHandler = async (event) => {
    ...
    console.log("[NewEntry] Form State:", formState.inputs);
    console.log("[NewEntry] User ID:", auth.userId);
    ...
    try {
      ...
      console.log("[NewEntry] Response:", responseData);

      
      navigate(`/${auth.userId}/journal`);
    } catch (err) {
      console.error("[NewEntry] Error creating entry:", err);
    }
  };
